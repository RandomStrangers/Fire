...
namespace Flames.Commands.Moderation
{
    public class CmdPass : Command2
    {
        ...
        public static DefaultPassAuthenticator defaultPassAuthenticator = new DefaultPassAuthenticator();
        public override void Use(Player p, string message, CommandData data)
        {
            if (data.Rank < Server.Config.VerifyAdminsRank)
            {
                Formatter.MessageNeedMinPerm(p, "+ can verify or set a password", Server.Config.VerifyAdminsRank); 
                return;
            }
        ...
        public static void VerifyPassword(Player p, string password)
        {
            ...
            if (!defaultPassAuthenticator.HasPassword(p.name))
            {
                p.Message("You have not &Wset a password, &Suse &T/SetPass [Password] &Wto set one!");
                return;
            }

            if (defaultPassAuthenticator.VerifyPassword(p.truename, password)) return;
        ...
        {
            if (p.Unverified && defaultPassAuthenticator.HasPassword(p.name))
            {
                defaultPassAuthenticator.RequiresVerification(p, "can change your password");
                p.Message("Forgot your password? Contact &W{0} &Sto &Wreset it.", Server.Config.OwnerName);
                return;
            }
            ...
            defaultPassAuthenticator.StorePassword(p.name, password);
        ...
        public void ResetPassword(Player p, string name, CommandData data)
        {
            ...
            if (p.Unverified)
            {
                defaultPassAuthenticator.RequiresVerification(p, "can reset passwords");
                return;
            }
            if (!CheckResetPerms(p, data)) return;

            if (defaultPassAuthenticator.ResetPassword(target))
            {
