...
namespace Flames.Modules.Relay.RelayDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to RelayDiscord API " + target, ex);
        }

        static void LogWarning(Exception ex)
        {
            Logger.Log(LogType.Warning, "Error sending request to RelayDiscord API - " + ex.Message);
        }
        ...
        static void LogResponse(string err)
        {
            ...
            Logger.Log(LogType.Warning, "RelayDiscord API returned: " + err);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "RelayDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "RelayDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_relay.txt",
                                        "// This file is used to replace words/phrases sent to RelayDiscord",
        ...
        {
            Controllers = PlayerList.Load("text/discord/controllers_relay.txt");
        }
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot_relay.properties";
    ...
    {
        public override string Name { get { return "RelayDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdRelayDiscordBot());
			Command.Register( new CmdRelayDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting RelayDiscord bot");
            Command.Unregister(Command.Find("RelayDiscordBot"));
			Command.Unregister(Command.Find("RelayDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdRelayDiscordBot : RelayBotCmd
    {
        public override string name { get { return "RelayDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdRelayDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "RelayDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "RelayDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("RelayDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("RelayDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "RelayDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
