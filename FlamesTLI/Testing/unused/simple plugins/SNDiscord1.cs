...
namespace Flames.Modules.Relay.SNDiscord1
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to SNDiscord1 API " + target, ex);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "SNDiscord1 bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "SNDiscord1Bot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_sn.txt",
                                        "// This file is used to replace words/phrases sent to SNDiscord1",
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot_sn1.properties";
        ...
        public void Save()
        {
            ...
            {
                w.WriteLine("# SNDiscord1 relay bot configuration");
    ...
    {
        public override string Name { get { return "SNDiscord1Relay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdSNDiscord1Bot());
			Command.Register( new CmdSNDiscord1Controllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting SNDiscord1 bot");
            Command.Unregister(Command.Find("SNDiscord1Bot"));
			Command.Unregister(Command.Find("SNDiscord1Controllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdSNDiscord1Bot : RelayBotCmd
    {
        public override string name { get { return "SNDiscord1Bot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdSNDiscord1Controllers : BotControllersCmd
    {
        public override string name { get { return "SNDiscord1Controllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "SNDiscord1 relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("SNDiscord1 relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("SNDiscord1 relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "SNDiscord1 relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
