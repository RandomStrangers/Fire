...
namespace Flames.Modules.Relay.SNDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to SNDiscord API " + target, ex);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "SNDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "SNDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_sn.txt",
                                        "// This file is used to replace words/phrases sent to SNDiscord",
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot_sn.properties";
    ...
    {
        public override string Name { get { return "SNDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdSNDiscordBot());
			Command.Register( new CmdSNDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting SNDiscord bot");
            Command.Unregister(Command.Find("SNDiscordBot"));
			Command.Unregister(Command.Find("SNDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdSNDiscordBot : RelayBotCmd
    {
        public override string name { get { return "SNDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdSNDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "SNDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "SNDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("SNDiscord relay: Invalid bot token provided - unable to connect");
            }
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "SNDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
