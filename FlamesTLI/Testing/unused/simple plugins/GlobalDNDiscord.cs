...
namespace Flames.Modules.Relay.GlobalDNDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to GlobalDNDiscord API " + target, ex);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "GlobalDNDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "GlobalDNDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_sn.txt",
                                        "// This file is used to replace words/phrases sent to GlobalDNDiscord",
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "globalchat/discordbot_dn.properties";
        ...
        public void Save()
        {
            ...
            {
                w.WriteLine("# GlobalDNDiscord relay bot configuration");
    ...
    {
        public override string Name { get { return "GlobalDNDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdGlobalDNDiscordBot());
			Command.Register( new CmdGlobalDNDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting GlobalDNDiscord bot");
            Command.Unregister(Command.Find("GlobalDNDiscordBot"));
			Command.Unregister(Command.Find("GlobalDNDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdGlobalDNDiscordBot : RelayBotCmd
    {
        public override string name { get { return "GlobalDNDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdGlobalDNDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "GlobalDNDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "GlobalDNDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("GlobalDNDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("GlobalDNDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "GlobalDNDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
