...
namespace Flames.Modules.Relay.GSDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to GSDiscord API " + target, ex);
        }

        static void LogWarning(Exception ex)
        {
            Logger.Log(LogType.Warning, "Error sending request to GSDiscord API - " + ex.Message);
        }
        ...
        static void LogResponse(string err)
        {
            ...
            Logger.Log(LogType.Warning, "GSDiscord API returned: " + err);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "GSDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "GSDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_gs.txt",
                                        "// This file is used to replace words/phrases sent to GSDiscord",
        ...
        {
            Controllers = PlayerList.Load("text/discord/controllers_gs.txt");
        }
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot_gs.properties";
    ...
    {
        public override string Name { get { return "GSDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdGSDiscordBot());
			Command.Register( new CmdGSDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting GSDiscord bot");
            Command.Unregister(Command.Find("GSDiscordBot"));
			Command.Unregister(Command.Find("GSDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdGSDiscordBot : RelayBotCmd
    {
        public override string name { get { return "GSDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdGSDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "GSDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "GSDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("GSDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("GSDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "GSDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
