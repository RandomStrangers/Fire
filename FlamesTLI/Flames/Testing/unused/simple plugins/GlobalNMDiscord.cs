...
namespace Flames.Modules.Relay.GlobalNMDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to GlobalNMDiscord API " + target, ex);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "GlobalNMDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "GlobalNMDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_nm.txt",
                                        "// This file is used to replace words/phrases sent to GlobalNMDiscord",
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "globalchat/discordbot_nm.properties";
        ...
        public void Save()
        {
            ...
            {
                w.WriteLine("# GlobalNMDiscord relay bot configuration");
    ...
    {
        public override string Name { get { return "GlobalNMDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdGlobalNMDiscordBot());
			Command.Register( new CmdGlobalNMDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting GlobalNMDiscord bot");
            Command.Unregister(Command.Find("GlobalNMDiscordBot"));
			Command.Unregister(Command.Find("GlobalNMDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdGlobalNMDiscordBot : RelayBotCmd
    {
        public override string name { get { return "GlobalNMDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdGlobalNMDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "GlobalNMDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "GlobalNMDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("GlobalNMDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("GlobalNMDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "GlobalNMDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
