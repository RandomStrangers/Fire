...
namespace Flames.Modules.Relay.GlobalNEDiscord
{
    ...
    public sealed class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.LogError("Error sending request to GlobalNEDiscord API " + target, ex);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res)
        {
            ...
            Logger.Log(LogType.SystemActivity, "GlobalNEDiscord bot ratelimited! Trying again in {0} seconds..", delay);
    ...
    public sealed class DiscordBot : RelayBot
    {
        ...
        public override string RelayName { get { return "GlobalNEDiscordBot"; } }
        ...
        TextFile replacementsFile = new TextFile("text/discord/replacements_sn.txt",
                                        "// This file is used to replace words/phrases sent to GlobalNEDiscord",
        ...
        {
            Controllers = PlayerList.Load("text/discord/controllers_sn.txt");
        }
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "globalchat/discordbot_ne.properties";
        ...
        public void Save()
        {
            ...
            {
                w.WriteLine("# GlobalNEDiscord relay bot configuration");
    ...
    {
        public override string Name { get { return "GlobalNEDiscordRelay"; } }
public override string Flames_Version { get { return "1.0.0.0"; } }
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register( new CmdGlobalNEDiscordBot());
			Command.Register( new CmdGlobalNEDiscordControllers());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        public override void Unload(bool shutdown)
        {
            ...
            Bot.Disconnect("Disconnecting GlobalNEDiscord bot");
            Command.Unregister(Command.Find("GlobalNEDiscordBot"));
			Command.Unregister(Command.Find("GlobalNEDiscordControllers"));
        }

        void OnConfigUpdated() { Bot.ReloadConfig(); }
    }

    public sealed class CmdGlobalNEDiscordBot : RelayBotCmd
    {
        public override string name { get { return "GlobalNEDiscordBot"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }

    public sealed class CmdGlobalNEDiscordControllers : BotControllersCmd
    {
        public override string name { get { return "GlobalNEDiscordControllers"; } }
        protected override RelayBot Bot { get { return DiscordPlugin.Bot; } }
    }
    ...
    public sealed class DiscordWebsocket : ClientWebSocket
    {
        ...
        {
            SentIdentify = false;
            if (readable) Logger.Log(LogType.SystemActivity, "GlobalNEDiscord relay bot closing: " + reason);
            Close();

            if (reason == REASON_INVALID_TOKEN)
            {
                CanReconnect = false;
                throw new InvalidOperationException("GlobalNEDiscord relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                ...
                CanReconnect = false;
                throw new InvalidOperationException("GlobalNEDiscord relay: Message Content Intent is not enabled in Bot Account settings, " +
        ...
        void DispatchPacket(int opcode, JsonObject obj)
        {
            ...
            else if (opcode == OPCODE_INVALID_SESSION)
            {
                ...
                Logger.Log(LogType.Warning, "GlobalNEDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
