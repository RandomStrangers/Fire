...
namespace Flames.Discord2
{
    public sealed class DiscordPlugin : Plugin
    {
        ...
        public override void Load(bool startup)
        {
            ...
            Command.Register(new CmdDiscordBot2());
            Command.Register(new CmdDiscordControllers2());
            Bot.Config = Config;
            Bot.ReloadConfig();
            Bot.Connect();
        ...
        {
            Command.Unregister(Command.Find("DiscordBot2"));
            Command.Unregister(Command.Find("DiscordControllers2"));
            OnConfigUpdatedEvent.Unregister(OnConfigUpdated);
            Bot.Disconnect("Disconnecting Discord2 bot");
        }
    ...
    public sealed class CmdDiscordBot2 : RelayBotCmd
    {
        public override string name
        {
            get
    ...
    public sealed class CmdDiscordControllers2 : BotControllersCmd
    {
        public override string name
        {
            get
            {
                return "DiscordControllers2";
            }
        }
        public override RelayBot Bot
        {
    ...
    public sealed class DiscordConfig
    {
        ...
        public const string PROPS_PATH = "properties/discordbot2.properties";
    ...
    public class DiscordApiClient : AsyncWorker<DiscordApiMessage>
    {
        ...
        public static void LogError(Exception ex, DiscordApiMessage msg)
        {
            ...
            Logger.Log(LogType.Error, "Error sending request to Discord2 API {0}: {1}", target, ex);
        }
        public static void LogWarning(Exception ex)
        {
            Logger.Log(LogType.Warning, "Error sending request to Discord2 API - {0}", ex.Message);
        }
        public static void LogResponse(string err)
        {
            ...
            Logger.Log(LogType.Warning, "Discord2 API returned: {0}", err);
        }
    ...
    public class DiscordBot : RelayBot
    {
        ...
        public TextFile replacementsFile = new TextFile("text/discord/replacements2.txt",
                                        "// This file is used to replace words/phrases sent to Discord2",
                                        "// Lines starting with // are ignored",
                                        "// Lines should be formatted like this:",
                                        "// example:http://example.org",
                                        "// That would replace 'example' in messages sent to Discord2 with 'http://example.org'");
        ...
        public override void DoDisconnect(string reason)
        {
            ...
            {
                Logger.Log(LogType.Error, "Error disconnecting Discord2 socket: {0}", ex);
            }
        }
        public override void ReloadConfig()
        {
            ...
            Logger.Log(LogType.Warning, "can-mention-everyone option is enabled in {0}, " +
                       "which allows pinging all users on Discord2 from in-game. " +
        ...
        public void DoUpdateStatus(SchedulerTask task)
        {
            ...
            {
                Logger.Log(LogType.Error, "Error updating Discord2 status: {0}", ex);
            }
        }
    ...
    public class DiscordWebsocket : ClientWebSocket
    {
        ...
        public override void Close()
        {
            ...
            {
                Logger.Log(LogType.Error, "Error closing Discord2 socket: {0}", ex);
            }
        }
        ...
        public override void OnDisconnected(int reason)
        {
            ...
            {
                CanReconnect = false;
                throw new InvalidOperationException("Discord2 relay: Invalid bot token provided - unable to connect");
            }
            else if (reason == REASON_DENIED_INTENT)
            {
                CanReconnect = false;
                throw new InvalidOperationException("Discord2 relay: Message Content Intent is not enabled in Bot Account settings, " +
