...
using Flames.RandomStrangersScripting;
using Flames.Events.ServerEvents;
using Flames.Modules.RandomStrangersCompiling;
using System.IO;
...
namespace Flames.Modules.RandomStrangersCompiling
{
    public class CmdRandomStrangersPluginCompile : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            CompileRandomStrangersPlugin(p, paths, compiler);
        }
        public virtual void CompileRandomStrangersPlugin(Player p, string[] paths, ICompiler compiler)
        {
            string dstPath = IScripting.RandomStrangersPluginPath(paths[0]);
            for (int i = 0; i < paths.Length; i++)
            {
                paths[i] = compiler.RandomStrangersPluginPath(paths[i]);
            }
            CompilerOperations.Compile(p, compiler, "RandomStrangersPlugin", paths, dstPath);
        }
    ...
    public class CmdRandomStrangersPluginCompLoad : CmdRandomStrangersPluginCompile
    {
        public override string name { get { return "RandomStrangersPluginCompLoad"; } }
        public override string shortcut { get { return "rscml"; } }
        public override CommandAlias[] Aliases
        {
            get
            {
                return new[] {
                    new CommandAlias("rspcompload"),
                    new CommandAlias("rspcml"),
                };
            }
        }
        public override void CompileRandomStrangersPlugin(Player p, string[] paths, ICompiler compiler)
        {
            string dst = IScripting.RandomStrangersPluginPath(paths[0]);
            UnloadRandomStrangersPlugin(p, paths[0]);
            base.CompileRandomStrangersPlugin(p, paths, compiler);
            ScriptingOperations.LoadRandomStrangersPlugins(p, dst);
        }
        public static void UnloadRandomStrangersPlugin(Player p, string name)
        {
            RandomStrangersPlugin RandomStrangersplugin = RandomStrangersPlugin.FindRandomStrangersCustom(name);
        ...
        public override void Help(Player p)
        {
            p.Message("&T/RandomStrangersPluginCompLoad [plugin]");
            p.Message("&HCompiles and loads (or reloads) a C# RandomStrangers plugin into the server");
        }
    }
    public class CmdRandomStrangersPlugin : Command
    {
        public override string name { get { return "RandomStrangersPlugin"; } }
        ...
        public override CommandAlias[] Aliases
        {
            get
            {
                return new[] { new CommandAlias("RSPLoad", "load"), new CommandAlias("RSPUnload", "unload"),
                    new CommandAlias("RSPlugins", "list") };
            }
        }
        public override bool MessageBlockRestricted { get { return true; } }
        public override void Use(Player p, string message)
        {
            ...
            if (IsListAction(args[0]))
            {
                ...
                p.Message("Loaded RandomStrangers plugins:");
                Paginator.Output(p, RandomStrangersPlugin.CustomRandomStrangersPlugins, pl => pl.name,
                                 "RandomStrangers", "randomstrangersplugins", modifier);
                return;
            }
            if (args.Length == 1)
            {
            ...
            {
                string path = IScripting.RandomStrangersPluginPath(name);
                ScriptingOperations.LoadRandomStrangersPlugins(p, path);
            }
            else if (cmd.CaselessEq("unload"))
            {
                UnloadRandomStrangersPlugin(p, name);
            }
            else if (cmd.CaselessEq("create"))
            {
                Find("RandomStrangersPluginCreate").Use(p, name);
            }
            else if (cmd.CaselessEq("compile"))
            {
                Find("RandomStrangersPluginCompile").Use(p, name);
            }
            else
            {
                Help(p);
        ...
        public override void Help(Player p)
        {
            p.Message("&T/RandomStrangersPlugin load [filename]");
            p.Message("&HLoad a compiled plugin from the &fRandomStrangers plugins &Hfolder");
            p.Message("&T/RandomStrangersPlugin unload [name]");
            p.Message("&HUnloads a currently loaded RandomStrangers plugin");
            p.Message("&T/RandomStrangersPlugin list");
            p.Message("&HLists all loaded RandomStrangers plugins");
        }
    }
    public class CmdRandomStrangersPluginCreate : CmdRandomStrangersPluginCompile
    {
        public override string name { get { return "RandomStrangersPluginCreate"; } }
        public override string shortcut { get { return "RSPCreate"; } }
        ...
        public override void CompileRandomStrangersPlugin(Player p, string[] paths, ICompiler compiler)
        {
            ...
            {
                CompilerOperations.CreateRandomStrangersPlugin(p, cmd, compiler);
            }
        }
    ...
    {
        public const string RANDOMSTRANGERS_PLUGINS_SOURCE_DIR = "randomstrangersplugins/";
        public const string ERROR_LOG_PATH = "logs/errors/compiler_rs.log";
        ...
        /// <summary> Returns source code for an example RandomStrangers plugin </summary>
        public abstract string RandomStrangersPluginSkeleton { get; }

        public string RandomStrangersPluginPath(string name)
        {
            return RANDOMSTRANGERS_PLUGINS_SOURCE_DIR + name + FileExtension;
        }
        ...
        /// <summary> Generates source code for an example RandomStrangers plugin, 
        /// preformatted with the given name and creator </summary>
        public string GenExampleRandomStrangersPlugin(string RandomStrangersplugin, string creator)
        {
            return FormatSource(RandomStrangersPluginSkeleton, RandomStrangersplugin, creator, Server.Version);
        }
        ...
        public ICompilerErrors Compile(string[] srcPaths, string dstPath, bool logErrors)
        {
            foreach (string path in srcPaths)
            {
                ...
                contents = contents.Replace(" : Plugin", " : RandomStrangersPlugin");
                contents = contents.Replace("using RandomStrangers", "using Flames");
                contents = contents.Replace("namespace RandomStrangers", "namespace Flames");
        ...
        public void AddReferences(string path, string commentPrefix, List<string> referenced)
        {
            ...
            using (StreamReader r = new StreamReader(path))
            {
                ...
                string newplgPrefix = commentPrefix + "randomstrangerspluginref ";
                string line;
                ...
                while ((line = r.ReadLine()) != null)
                {
                    ...
                    {
                        path = Path.Combine(IScripting.RANDOMSTRANGERS_PLUGINS_DLL_DIR, GetDLL(line));
    ...
    public class CSCompiler : ICompiler
    {
        ...
        public override string RandomStrangersPluginSkeleton
        {
            get
            {
...
namespace Flames
{{
\tpublic class {0} : RandomStrangersPlugin
\t{{
\t\t// The RandomStrangers plugin's name (i.e what shows in /RSPlugins)
\t\tpublic override string name {{ get {{ return ""{0}""; }} }}

\t\t// The oldest version of Flames this RandomStrangers plugin is compatible with
\t\tpublic override string Flames_Version {{ get {{ return ""{2}""; }} }}

\t\t// Message displayed in server logs when this RandomStrangers plugin is loaded
\t\tpublic override string welcome {{ get {{ return ""Loaded Message!""; }} }}

\t\t// Who created/authored this RandomStrangers plugin
\t\tpublic override string creator {{ get {{ return ""{1}""; }} }}

\t\t// Called when this RandomStrangers plugin is being loaded (e.g. on server startup)
\t\tpublic override void Load(bool startup)
\t\t{{
\t\t\t//code to hook into events, load state/resources etc goes here
\t\t}}

\t\t// Called when this RandomStrangers plugin is being unloaded (e.g. on server shutdown)
\t\tpublic override void Unload(bool shutdown)
\t\t{{
\t\t\t//code to unhook from events, dispose of state/resources etc goes here
\t\t}}

\t\t// Displays help for or information about this RandomStrangers plugin
\t\tpublic override void Help(Player p)
\t\t{{
\t\t\tp.Message(""No help is available for this RandomStrangers plugin."");
\t\t}}
\t}}
}}";
            }
    ...
    public static class CompilerOperations
    {
        ...
        /// <param name="type"> Type of files being compiled (e.g. RandomStrangers plugin) </param>
    ...
    public class RandomStrangersCompilerPlugin : RandomStrangersPlugin
    {
        public override string name { get { return "RandomStrangersCompiler"; } }

        public Command RandomStrangersCmdCreate = new CmdRandomStrangersPluginCreate();
        public Command RandomStrangersCmdCompile = new CmdRandomStrangersPluginCompile();
        public Command RandomStrangersCmdCompLoad = new CmdRandomStrangersPluginCompLoad();
        public Command RandomStrangersCmdPlugin = new CmdRandomStrangersPlugin();

        public override void Load(bool startup)
        {
            Server.EnsureDirectoryExists(ICompiler.RANDOMSTRANGERS_PLUGINS_SOURCE_DIR);
            Command.Register(RandomStrangersCmdCreate, RandomStrangersCmdCompile, RandomStrangersCmdCompLoad, RandomStrangersCmdPlugin);
        }

        public override void Unload(bool shutdown)
        {
            Command.Unregister(RandomStrangersCmdCreate, RandomStrangersCmdCompile, RandomStrangersCmdCompLoad, RandomStrangersCmdPlugin);
        }
    }
}
namespace Flames.RandomStrangersScripting
{
    public static class ScriptingOperations
    {
        public static bool LoadRandomStrangersPlugins(Player p, string path)
        {
            if (!File.Exists(path))
            {
                p.Message("File &9{0} &Snot found.", path);
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError("Error loading RandomStrangers plugins from " + path, ex);
                return false;
            }
        }

        public static bool UnloadRandomStrangersPlugin(Player p, RandomStrangersPlugin RandomStrangersplugin)
        {
            if (!RandomStrangersPlugin.Unload(RandomStrangersplugin))
            {
                p.Message("&WError unloading RandomStrangers plugin. See error logs for more information.");
                return false;
            }

            p.Message("RandomStrangers plugin {0} &Sunloaded successfully", RandomStrangersplugin.name);
            return true;
        }
    }
    /// <summary> Exception raised when attempting to load a RandomStrangers plugin 
    /// that has the same name as an already loaded RandomStrangers plugin </summary>
    ...
    /// <summary> Utility methods for loading assemblies, and RandomStrangers plugins </summary>
    public static class IScripting
    {
        ...
        public static Assembly ResolveRandomStrangersPluginReference(string name)
        {
            return null;
        }

        public const string RANDOMSTRANGERS_PLUGINS_DLL_DIR = "randomstrangersplugins/";

        /// <summary> Returns the default .dll path for the RandomStrangers plugin with the given name </summary>
        public static string RandomStrangersPluginPath(string name)
        {
            return RANDOMSTRANGERS_PLUGINS_DLL_DIR + name + ".dll";
        }


        public static void Init()
        {
            Directory.CreateDirectory(RANDOMSTRANGERS_PLUGINS_DLL_DIR);
            AppDomain.CurrentDomain.AssemblyResolve += ResolveRandomStrangersPluginAssembly;
        }

        // only used for resolving RandomStrangers plugin DLLs depending on other RandomStrangers plugin DLLs
        public static Assembly ResolveRandomStrangersPluginAssembly(object sender, ResolveEventArgs args)
        {
            // This property only exists in .NET framework 4.0 and later
            Assembly requestingAssembly = args.RequestingAssembly;

            if (requestingAssembly == null) return null;
            if (!IsRandomStrangersPluginDLL(requestingAssembly)) return null;

            Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies();
            foreach (Assembly assem in assemblies)
            {
                if (!IsRandomStrangersPluginDLL(assem)) continue;

                if (args.Name == assem.FullName) return assem;
            }

            Assembly coreRef = ResolveRandomStrangersPluginReference(args.Name);
            if (coreRef != null) return coreRef;

            Logger.Log(LogType.Warning, "Custom RandomStrangers plugin [{0}] tried to load [{1}], but it could not be found",
                       requestingAssembly.FullName, args.Name);
            return null;
        }

        public static bool IsRandomStrangersPluginDLL(Assembly a)
        {
        ...
        public static string DescribeLoadError(string path, Exception ex)
        {
            ...
            // p.Message("&WError loading RandomStrangers plugin. See error logs for more information.");
        }


        public static void AutoloadRandomStrangersPlugins()
        {
            string[] files = AtomicIO.TryGetFiles(RANDOMSTRANGERS_PLUGINS_DLL_DIR, "*.dll");
            if (files == null) return;

            // Ensure that RandomStrangers plugin files are loaded in a consistent order,
            //  in case RandomStrangers plugins have a dependency on other RandomStrangers plugins
            Array.Sort(files);
            ...
            foreach (string path in files)
            {
                try
                {
                    LoadRandomStrangersPlugin(path, true);
                }
                catch (Exception ex)
                {
                    Logger.LogError("Error loading RandomStrangers plugins from " + path, ex);
        ...
        /// <summary> Loads all RandomStrangers plugins from the given .dll path. </summary>
        public static List<RandomStrangersPlugin> LoadRandomStrangersPlugin(string path, bool auto)
        {
...
namespace Flames
{
    public class RandomStrangersPluginLoader : Plugin
    {
        ...
        public static RandomStrangersCompilerPlugin randomstrangerscompilerplugin = new RandomStrangersCompilerPlugin();
        public override void Load(bool startup)
        {
            RandomStrangersPlugin.LoadCoreRandomStrangersPlugin(randomstrangerscompilerplugin, startup);
            RandomStrangersPlugin.LoadAll();
            OnShuttingDownEvent.Register(OnShutdown, Priority.Critical);
        }
        public void OnShutdown(bool restarting, string message)
        {
            RandomStrangersPlugin.UnloadAll();
        }
        public override void Unload(bool shutdown)
        {
            RandomStrangersPlugin.UnloadAll();
            RandomStrangersPlugin.UnloadRandomStrangersPlugin(randomstrangerscompilerplugin, shutdown);
...
namespace Flames
{
    ...
    public abstract class RandomStrangersPlugin
    {
        ...
        /// <param name="auto"> True if RandomStrangers plugin is being automatically loaded (e.g. on server startup), false if manually. </param>
        public abstract void Load(bool auto);

        /// <summary> Unhooks from events and disposes of state/resources etc </summary>
        /// <param name="auto"> True if RandomStrangers plugin is being auto unloaded (e.g. on server shutdown), false if manually. </param>
        public abstract void Unload(bool auto);

        /// <summary> Called when a player does /Help on the RandomStrangers plugin. Typically tells the player what this RandomStrangers plugin is about. </summary>
        ...
        /// <summary> Name of the RandomStrangers plugin. </summary>
        public abstract string name { get; }
        /// <summary> The oldest version of Flames this RandomStrangers plugin is compatible with. </summary>
        ...
        /// <summary> Version of this RandomStrangers plugin. </summary>
        public virtual int build { get { return 0; } }
        /// <summary> Message to display once this RandomStrangers plugin is loaded. </summary>
        public virtual string welcome { get { return ""; } }
        /// <summary> The creator/author of this RandomStrangers plugin. (Your name) </summary>
        public virtual string creator { get { return ""; } }
        /// <summary> Whether or not to auto load this RandomStrangers plugin on server startup. </summary>
        public virtual bool LoadAtStartup { get { return true; } }


        /// <summary> List of RandomStrangers plugins/modules included in the server software </summary>
        public static List<RandomStrangersPlugin> CoreRandomStrangersPlugins = new List<RandomStrangersPlugin>();
        public static List<RandomStrangersPlugin> CustomRandomStrangersPlugins = new List<RandomStrangersPlugin>();


        public static RandomStrangersPlugin FindRandomStrangersCustom(string name)
        {
            foreach (RandomStrangersPlugin pl in CustomRandomStrangersPlugins)
            {
        ...
        {
            for (int i = 0; i < CustomRandomStrangersPlugins.Count; i++)
            {
                UnloadRandomStrangersPlugin(CustomRandomStrangersPlugins[i], true);
            }
            CustomRandomStrangersPlugins.Clear();

            for (int i = 0; i < CoreRandomStrangersPlugins.Count; i++)
            {
                UnloadRandomStrangersPlugin(CoreRandomStrangersPlugins[i], true);
            }
        }
        public static void LoadAll()
        {
            LoadCoreRandomStrangersPlugin(new RandomStrangersCompilerPlugin());
            IScripting.AutoloadRandomStrangersPlugins();
        }
        public static void LoadCoreRandomStrangersPlugin(RandomStrangersPlugin RandomStrangersplugin, bool startup)
        {
            LoadCoreRandomStrangersPlugin(RandomStrangersplugin);
        }
        public static void LoadCoreRandomStrangersPlugin(RandomStrangersPlugin RandomStrangersplugin)
        {
            List<string> disabled = Server.Config.DisabledModules;
            if (disabled.CaselessContains(RandomStrangersplugin.name)) return;
            RandomStrangersplugin.Load(true);
            CoreRandomStrangersPlugins.Add(RandomStrangersplugin);
        }
    }
}
#endif