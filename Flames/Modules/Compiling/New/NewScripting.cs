...
namespace Flames.NewScripting
{
    ...
    public static class IScripting
    {
        ...
        public static List<NewPlugin> LoadNewPlugin(string path, bool auto)
        {
            ...
            foreach (NewPlugin npl in newplugins)
            {
                if (NewPlugin.FindNewCustom(npl.name) != null)
                    throw new AlreadyLoadedException("New plugin " + npl.name + " is already loaded");

                NewPlugin.Load(npl, auto);
            }
            return newplugins;
        }
    }
