...
namespace Flames.Eco
{
    public sealed class TitleItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string title)
        {
            if (title.Length == 0)
            {
                ...
                p.Message("&aYour title was removed for free."); 
                return;
            }
            ...
            if (title == p.title)
            {
                p.Message("&WYou already have that title.");
                return;
            }
    ...
    public sealed class NickItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string nick)
        {
            if (nick.Length == 0)
            {
                ...
                p.Message("&aYour nickname was removed for free."); 
                return;
            }
            ...
            {
                p.Message("&WYou already have that nickname."); 
                return;
            }
            if (nick.Length >= 30)
            {
                p.Message("&WNicknames must be under 30 characters.");
                return;
            }
    ...
    public sealed class TitleColorItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string args)
        {
            ...
            {
                p.Message("&WYour title color is already " + color + name); 
                return;
            }
    ...
    public sealed class ColorItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string args)
        {
            ...
            if (color == p.color)
            {
                p.Message("&WYour color is already " + color + name); 
                return;
            }
