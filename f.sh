...
fdir="./"							##  Folder containing FlamesCLI.exe
...
gamename="RedCraft - Flames"		##  Arbitrary name of server, will not affect actual server name
gamepid="${fdir}/f.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${fdir}/f.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${fdir}/FlamesCLI.exe" ]; then
	echo -e "cannot find ${fdir}/FlamesCLI.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${fdir}/FlamesCLI.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${fdir}/FlamesCLI.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${fdir}/FlamesCLI.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${fdir}/FlamesCLI.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${fdir}/FlamesCLI.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${fdir}/Flames_.update" ]; then
			if [ -f "${fdir}/FlamesCLI.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${fdir}/FlamesCLI.exe
				rm ${fdir}/Flames_.dll
				mv ${fdir}/FlamesCLI.update ${fdir}/FlamesCLI.exe
				mv ${fdir}/Flames_.update ${fdir}/Flames_.dll
				if [ -f "${fdir}/Flames_.update" ]; then
					if [ -f "${fdir}/FlamesCLI.update" ]; then
						echo -e FAILED!
						if [ -f "${fdir}/Flames_.dll" ]; then
							if [ -f "${fdir}/FlamesCLI.exe" ]; then
								$0 ${default}
							fi
						else
